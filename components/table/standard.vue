    ...
    <div v-else>
        <v-card-title>
            ...
            <v-btn small class="mx-1" v-if="config.globalActions && config.globalActions.length === 1"
                ...
                <v-icon v-if="config.globalActions[0].icon" :left="!!config.globalActions[0].text">
                    {{ config.globalActions[0].icon }}
                </v-icon>
                {{ config.globalActions[0].text }}
            </v-btn>
            <v-btn v-if="selectedRows.length" small outlined class="mx-1" color="danger" @click="
...
selectedRows = [];
        ...
        <v-dialog v-model="customizeViewShow" max-width="300">
            <v-card>
                ...
                <v-card-text>
                    <div v-for="header in config.headers.filter(
