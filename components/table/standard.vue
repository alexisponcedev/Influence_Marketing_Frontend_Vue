    ...
    <div v-else>
        <v-card-title>
            ...
            <v-btn
                small
                class="mx-1"
                v-if="config.globalActions && config.globalActions.length <= 2"
                v-for="(act, index) in config.globalActions"
                :key="index"
                :color="act.color"
                :class="act.class"
                @click="act.action ? $emit(act.action) : undefined"
                :to="act.to"
            >
                <v-icon v-if="act.icon" :left="!!act.text">
                    {{ act.icon }}
                </v-icon>
                {{ act.text }}
            </v-btn>
...
export default class StandardTable extends Vue {
    ...
    @Prop({ default: "Remove", type: String })
