<template>
    ...
    >
        <div class="tw-space-y-1.5">
            ...
            >
                <img
                    src="~/assets/images/meta/google-logo.png"
                    width="72px"
                    alt="Google Logo"
                />
            </div>
            ...
            <div class="tw-p-2 tw-rounded-lg tw-bg-white tw-space-y-2">
                ...
                <div>
                    ...
                    <div class="tw-text-green-800 tw-font-sm">
                        {{ `https://hisese-usa.com${model.route}` }}
                    </div>
                    <div
                        v-if="googleDescriptionIndex >= 0"
                        class="tw-text-gray-500 tw-font-sm"
                    >
            ...
            >
                ...
                <textarea
                    ...
                    class="x-input tw-flex-1"
                    v-model="model.meta[googleDescriptionIndex].content"
                ></textarea>
            </div>

            <div
                v-if="googleRobotIndex >= 0"
                class="tw-bg-gray-50 tw-overflow-hidden tw-border tw-border-solid tw-border-gray-200 tw-rounded-lg tw-flex twl-items-center"
            >
                <div
                    class="tw-px-3 tw-w-40 tw-font-semibold tw-flex tw-items-center tw-justify-start"
                ...
                <select
                    v-model="model.meta[googleRobotIndex].content"
                    class="x-input tw-flex-1"
                >
        ...
        <div class="tw-space-y-1.5">
            ...
            >
                <img
                    src="~/assets/images/meta/facebook-logo.png"
                    width="72px"
                    alt="Facebook Logo"
                />
            </div>
            ...
            <div class="tw-p-2 tw-rounded-lg tw-bg-white tw-space-y-2">
                ...
                <div
                    class="tw-border tw-border-solid tw-border-gray-200 tw-bg-gray-50"
                >
                    <img
                        v-if="
                            ogImageIndex > 0 && model.meta[ogImageIndex].content
                        "
            ...
            >
                ...
                <textarea
                    ...
                    placeholder="Enter description"
                    class="x-input tw-flex-1"
                    v-model="model.meta[ogDescriptionIndex].content"
                ></textarea>
            </div>

            <div
                v-if="ogSiteNameIndex >= 0"
                class="tw-bg-gray-50 tw-overflow-hidden tw-border tw-border-solid tw-border-gray-200 tw-rounded-lg tw-flex twl-items-center"
            >
                <div
                    class="tw-px-3 tw-w-40 tw-font-semibold tw-flex tw-items-center tw-justify-start"
        ...
        <div class="tw-space-y-1.5">
            <div
                class="tw-px-2 tw-py-2 tw-flex tw-justify-between tw-items-center tw-rounded-t-lg tw-bg-gray-200"
                style="border-bottom: 2px solid black"
            >
                <div>Custom Meta</div>
                <button
                    class="tw-bg-white tw-rounded tw-font-sm tw-px-2 tw-py-0.5 tw-shadow"
            ...
            <div
                v-for="(item, j) in customTags"
                class="tw-bg-gray-50 tw-overflow-hidden tw-border tw-border-solid tw-border-gray-200 tw-rounded-lg tw-flex twl-items-center"
            >
                <input
                    type="text"
                    placeholder="Enter meta name"
                    class="x-gray-input tw-w-40"
                    v-model="item.name"
                />
                <input
                    type="text"
                ...
                <button
                    class="tw-bg-white"
                    style="width: 30px"
                    @click.prevent="removeCustomTag(item)"
                >
...
export default class TextMetaFormField extends Vue {
    ...
    removeCustomTag(item: any) {
        this.model.meta.splice(
            this.model.meta.findIndex(
                (i: MetaInterface) =>
                    i.rel === item.rel &&
                    i.name === item.name &&
    ...
    get googleTitleIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel === "blank" && i.name === "title"
        );
    ...
    get googleDescriptionIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel === "blank" && i.name === "description"
        );
    ...
    get googleRobotIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel === "blank" && i.name === "robots"
        );
    ...
    get ogSiteNameIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel && i.rel.includes("og:site_name")
        );
        return index >= 0
            ? index
            : this.addRow(
    ...
    get ogTitleIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel && i.rel.includes("og:title")
        );
        return index >= 0
            ? index
            : this.addRow('property="og:title"', 'property="og:title"', "");
    }

    get ogDescriptionIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel && i.rel.includes("og:description")
        );
        return index >= 0
            ? index
            : this.addRow(
    ...
    get ogUrlIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel && i.rel.includes("og:url")
        );
        return index >= 0
            ? index
            : this.addRow('property="og:url"', 'property="og:url"', "");
    }

    get ogTypeIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel && i.rel.includes("og:type")
        );
        return index >= 0
            ? index
            : this.addRow('property="og:type"', 'property="og:type"', "");
    }

    get ogLocaleIndex() {
        let index = this.model.meta.findIndex(
            (i: MetaInterface) => i.rel && i.rel.includes("og:locale")
        );
        return index >= 0
            ? index
            : this.addRow('property="og:locale"', 'property="og:locale"', "");
    }

    get customTags() {
        return this.model.meta.filter(
            (i: MetaInterface) =>
                i.rel === "blank" &&
                i.name !== "title" &&
                i.name !== "description" &&
    ...
    onPageTitleChanged() {
        if (this.model.meta)
            this.model.meta!.forEach((item: any) => {
                if (item.rel && item.rel.includes("og:title"))
                    item.content = this.model.title;
                if (item.rel && item.rel === "blank" && item.name === "title")
                    item.content = this.model.title;
            });
    }
        ...
        if (this.model.meta && Array.isArray(this.model.meta))
            this.model.meta!.forEach((item: any) => {
                if (item.rel && item.rel.includes("og:url"))
                    item.content = "https://hisense-usa.com" + this.model.route;
            });
    }
    ...
    async getProductImage() {
        if (
            ...
            (this.model.model_type === "product" ||
                this.model.model_type === "support") &&
            !this.model.meta[this.ogImageIndex].content
        ) {
            let res = await this.$axios.$get(
                "https://impim.dev-api.hisenseportal.com/api/cms/getProduct/" +
                    this.model.model_id
            );
            if (res && res.data)
