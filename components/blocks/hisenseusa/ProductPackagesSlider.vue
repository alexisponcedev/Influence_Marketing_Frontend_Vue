    ...
    <div class="tw-p-3" v-if="!isEmpty">
        ...
        <div class="tw-mt-1 tw-flex tw-items-center tw-space-x-2" v-if="correctPage">
            ...
            <div class="tw-flex-1">
                ...
                <div class="tw-font-xs">total items :
                    <strong> {{ model.list.value.serverData.length - model.list.value.items.length }} </strong>
                </div>
            </div>
            ...
            <div class="tw-flex-1">
                <div
                    class="tw-h-40 tw-p-2 tw-border tw-border-solid tw-border-gray-300 tw-rounded-lg tw-overflow-y-auto">
                    <div v-for="(item , index) in model.list.value.items"
                         :key="id" @click="model.list.value.items.splice(index , 1)"
                         class="tw-flex tw-items-center tw-space-x-2 tw-py-0.5 tw-transition hover:tw-bg-gray-50 hover:tw-text-blue-500">
                        <div class="tw-h-10 tw-w-10 tw-text-center">
                            <img v-if="item.image" :src="item.image" class="tw-max-h-full tw-object-cover"
                                 alt="product image"/>
                ...
                <div class="tw-font-xs">selected items : <strong>{{ model.list.value.items.length }}</strong></div>
            </div>
