<template>
    <div class="pa-12" v-if="!isEmpty">
        ...
        <div class="products-container tw-space-y-2">
            <draggable v-model="model.list.value" group="product-list-black-friday">
...
<!--                        </div>-->
                        <div class="tw-flex-1">
                            <div class="tw-flex tw-items-center" v-for="(serie , index) in product.series">
                                <div class="tw-text-gray-600 tw-w-52 tw-text-right">{{ serie.title }}</div>
                                <form-field-text class=""
                                                 :field="{ label: 'Title ' + (index + 1), placeholder: '', rules: [], colAttrs: {cols: 4}}"
        ...
        this.model.list.value.push({
            ...
            series: product.productSeries.length > 0 ?
                product.productSeries[0].values.map((i: any) => {
                    return {header: '', title: i.title, id: i.products[0], price: 0, discount: 0, final_price: 0}
                }) :
                [{header: product.name, title: product.name, id: product.id, price: 0, discount: 0, final_price: 0}]
            ,
            price: 0,
            discount: 0,
        });
