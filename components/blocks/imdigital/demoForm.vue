<template>
    <div
        class="tw-grid tw-grid-cols-2 tw-gap-5 tw-px-9 tw-items-start tw-bg-[url(/blocks/demoForm.svg)] tw-bg-cover tw-py-[100px]"
        v-if="!isEmpty"
    >
        <div class="tw-flex tw-flex-col tw-items-start h-full">
            <h4
                class="tw-text-white tw-font-bold tw-text-[40px] tw-m-0 tw-mb-2"
            >
                {{ model?.title?.value }}
            </h4>
            <div
                class="tw-text-[#CCE2FF] tw-text-lg"
                v-html="model?.description?.value"
            ></div>

            <div class="tw-flex tw-gap-2 tw-mt-10">
                <img :src="model?.popularImage?.src" alt="icon" />
                <span class="tw-text-lg tw-font-semibold tw-text-[#3DF110]">{{
                    model?.popularText?.value
                }}</span>
            </div>
            <div class="tw-grid tw-grid-cols-2 tw-gap-x-4 tw-gap-y-5 tw-mt-10">
                <div
                    ...
                    :key="index"
                >
                    <h2 class="tw-text-[32px] tw-font-bold tw-text-white">
                        {{ item.Title.value }}
                    </h2>
                    <div
        ...
        <div class="">
            <div
                class="tw-px-9 tw-grid tw-grid-cols-2 tw-gap-[14px] tw-bg-white tw-rounded-[25px] tw-p-5"
            >
                <div
                    class="tw-bg-[#f5f6f6] tw-h-[44px] tw-relative tw-rounded-md tw-overflow-hidden"
                >
                    <input
                        type="text"
                        class="tw-w-full tw-h-full tw-px-4 tw-bg-[#f5f6f6] tw-pt-4 tw-outline-none"
                        placeholder="Text"
                    />
                    <span
                        class="tw-absolute tw-top-[5px] tw-left-4 tw-text-[#035282] tw-text-[10px] tw-font-semibold"
                        >First Name*</span
                    >
                </div>
                <div
                ...
                <div
                    class="tw-bg-[#f5f6f6] tw-h-[44px] tw-relative tw-rounded-md tw-overflow-hidden tw-col-span-2"
                >
                    ...
                    <select
                        ...
                        id="cars"
                        class="tw-w-full tw-h-full tw-px-4 tw-bg-[#f5f6f6] tw-pt-4 tw-outline-none"
                        placeholder="IMDigital"
                    >
                        <option value="volvo">Test</option>
                    </select>
                    <span
                        class="tw-absolute tw-top-[5px] tw-left-4 tw-text-[#035282] tw-text-[10px] tw-font-semibold"
                        >Product of interest*</span
                    >
                </div>
                ...
                <div class="tw-col-span-2">
                    ...
                    <button class="tw-inline-flex tw-text-sm tw-text-[#1042F1]">
                        {{ model?.link?.value }}
                    </button>
                </div>
                <button
                    class="tw-h-[48px] tw-rounded-md tw-col-span-2 tw-border tw-border-solid tw-border-[#3df110] tw-text-[#3df110] tw-mt-[60px]"
                >
                    {{ model?.demoBtn?.value }}
                </button>
            </div>
            <div class="tw-text-white tw-mt-5 tw-text-center tw-text-lg">
                {{ model?.join?.value }}
            </div>
        </div>
    </div>
