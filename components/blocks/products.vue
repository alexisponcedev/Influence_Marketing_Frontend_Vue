<template>
    <div
        class="tw-grid tw-grid-cols-2 tw-items-end tw-py-[75px]"
        v-if="!isEmpty"
    >
        <div
            class="tw-flex tw-flex-col tw-items-start tw-justify-end h-full tw-px-9"
        >
            <h4
                class="tw-text-[#030d30] tw-font-semibold tw-text-[32px] tw-m-0"
            >
                {{ model.smallTitle.value }}
            </h4>
            <h2 class="tw-text-[#030d30] tw-text-lg tw-font-semibold tw-m-0">
                {{ model.text.value }}
            </h2>
            <p
                class="tw-text-[#030d30] tw-text-lg tw-mt-[10px]"
                v-html="model.description.value"
            />
            <div class="tw-flex tw-items-center tw-gap-[32px]">
                <button
                    class="tw-mt-3 tw-text-[#1042f1] tw-text-lg tw-font-semibold tw-rounded-md tw-flex tw-items-center tw-gap-2"
                >
                    {{ model.more.value }}
